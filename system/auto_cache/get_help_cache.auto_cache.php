...
class get_help_cache_auto_cache extends auto_cache
{
	...
	public function load($param)
	{
		...
		if($help_list===false)
		{
			...
			$help_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."article_cate where type_id = 1 and is_effect=1 and is_delete = 0 and pid=0 order by sort desc limit ".intval(app_conf("HELP_CATE_LIMIT")));
			foreach($help_list as $k=>$v)
			{
				...
				$help_cate_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."article where cate_id in (".implode(",",$ids).") and is_effect=1  and is_delete = 0 order by sort desc limit ".intval(app_conf("HELP_ITEM_LIMIT")));
