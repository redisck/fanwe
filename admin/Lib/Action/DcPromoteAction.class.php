...
class DcPromoteAction extends CommonAction{
	...
	public function index()
	{
		...
		$db_modules = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."dc_promote");
	...
	public function install()
	{
		...
		if(file_exists($file))
		{
			...
			$rs = M("DcPromote")->where("class_name = '".$class_name."'")->count();
			if($rs > 0)
			{
		...
		$data['sort'] = (M("DcPromote")->max("sort") + 1);	
	...
	public function add_dc_promote()
	{
		$this->display();
	}
	...
	public function set_sort()
	{
		...
		$log_info = M("DcPromote")->where("id=".$id)->getField("class_name");
		if(!check_sort($sort))
		{
			$this->error(l("SORT_FAILED"),1);
		}
		M("DcPromote")->where("id=".$id)->setField("sort",$sort);
