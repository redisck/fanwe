...
class wxfocusModule extends BizBaseModule
{
    private $faces = array();
    private $navs = array();
    ...
    public function index()
    {
        ...
        $reply = array();
        $reply = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "weixin_reply where type=4 and account_id = ".$this->supplier_id);
        if($reply){
            // 以o_msg_type类型来判断当前的默认回复内容
            if($reply['o_msg_type'] == "news"){
                app_redirect(url("biz","wxfocus#dnews"));
            }else{
                app_redirect(url("biz","wxfocus#dtext"));
            }
        }else{
            app_redirect(url("biz","wxfocus#dtext"));
        }
	...
	public function dtext()
	{
		...
		$GLOBALS['tmpl']->assign("page_title", "关注时回复(文本) ");
        ...
        $GLOBALS['tmpl']->display("pages/wxfocus/index.html");
	}
    ...
    public function dnews(){
        ...
        $GLOBALS['tmpl']->assign("page_title", "关注时回复(图文)");
        $GLOBALS['tmpl']->display("pages/wxfocus/dnews.html");
